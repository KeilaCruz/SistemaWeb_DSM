...
export function CalendarioHome({especialidad}) {
    ...
    const fetchData = async () => {
      try {
        ...
        const eventsData = await Promise.all(citasData.map(async (cita) => {
          if (cita.datos_cita.especialidad === especialidad) { // Filtrar por especialidad
          ...
          return null; // Devolver null para citas que no sean de la especialidad deseada
        }));
        setEvents(eventsData.filter(event => event !== null)); // Eliminar eventos nulos
      } catch (error) {
